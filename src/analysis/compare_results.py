...
def get_latest_metrics(pipeline_type):
    ...
    patterns = [f"metrics_{pipeline_type}_visual_*.csv", f"metrics_{pipeline_type}_*.csv"]
    all_files = []
...
def plot_comparison(fl_file=None, single_file=None):
    ...
    if not fl_file or not single_file:
        print(f"Error: Missing metrics files. Found FL: {fl_file}, Single: {single_file}")
        return
    ...
    eval_fl = df_fl[(df_fl['mode'] == 'EVAL') & (df_fl['twin_id'].str.contains('global', case=False))].copy()
    if eval_fl.empty:
        # Fallback to average of all EVALs per round if global not found
        eval_fl = df_fl[df_fl['mode'] == 'EVAL'].groupby('round')['reward'].mean().reset_index()
    
    eval_single = df_single[(df_single['mode'] == 'EVAL') & (df_single['twin_id'].str.contains('global', case=False))].copy()
    if eval_single.empty:
        eval_single = df_single[df_single['mode'] == 'EVAL'].groupby('round')['reward'].mean().reset_index()
    ...
    plt.plot(eval_fl['round'], eval_fl['reward'], marker='o', linewidth=3, markersize=8, label='Federated Learning (Global)', color='#2ecc71')
    plt.plot(eval_single['round'], eval_single['reward'], marker='s', linewidth=3, markersize=8, label='Single Agent (Baseline)', color='#e74c3c')

    # Labels and details
    plt.title('Performance Comparison: Federated vs. Single Agent', fontsize=16, fontweight='bold', pad=20)
    ...
    if not eval_fl.empty:
        plt.annotate(f"{eval_fl['reward'].iloc[-1]:.2f}", 
                     xy=(eval_fl['round'].iloc[-1], eval_fl['reward'].iloc[-1]),
                     xytext=(5, 5), textcoords='offset points', color='#27ae60', fontweight='bold')
    
    if not eval_single.empty:
        plt.annotate(f"{eval_single['reward'].iloc[-1]:.2f}", 
                     xy=(eval_single['round'].iloc[-1], eval_single['reward'].iloc[-1]),
                     xytext=(5, -15), textcoords='offset points', color='#c0392b', fontweight='bold')
