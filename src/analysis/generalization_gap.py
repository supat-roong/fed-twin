...
def plot_generalization_gap(pipeline_type="single", metrics_file=None):
    ...
    train_env_eval = eval_df[~eval_df['twin_id'].str.contains('global', case=False)].copy()
    global_env_eval = eval_df[eval_df['twin_id'].str.contains('global', case=False)].copy()
    ...
    if not train_env_stats.empty:
        plt.plot(train_env_stats['round'], train_env_stats['reward'], 
                marker='o', linewidth=3, markersize=8, 
                label='Eval on Training Environment', color='#3498db', alpha=0.8)
    
    if not global_env_stats.empty:
        plt.plot(global_env_stats['round'], global_env_stats['reward'], 
                marker='s', linewidth=3, markersize=8, 
                label='Eval on Neutral Environment', color='#e74c3c', alpha=0.8)
    
    # Fill area between to show gap
    if not train_env_stats.empty and not global_env_stats.empty:
        # Align rounds
        merged = pd.merge(train_env_stats, global_env_stats, on='round', suffixes=('_train', '_global'))
        plt.fill_between(merged['round'], merged['reward_train'], merged['reward_global'],
                        alpha=0.2, color='gray', label='Generalization Gap')
    
    # Styling
    plt.title('Generalization Analysis: Training vs. Neutral Environment', 
