...
def plot_worker_diversity(fl_file=None):
    ...
    local_eval = eval_df[~eval_df['twin_id'].str.contains('global', case=False)].copy()
    ...
    for i, twin in enumerate(twins):
        ...
        plt.plot(twin_data['round'], twin_data['reward'], 
                 marker='', color=colors[i], alpha=0.3, linewidth=1, 
    ...
    plt.fill_between(train_stats['round'], 
                     train_stats['mean'] - train_stats['std'], 
                     train_stats['mean'] + train_stats['std'], 
                     color='blue', alpha=0.1, label='Worker Training Variance')
    plt.plot(train_stats['round'], train_stats['mean'], color='blue', 
             linewidth=2, label='Mean Local Training Reward')

    # 3. Plot Global Evaluation (High Emphasis)
    if not global_eval.empty:
        global_eval = global_eval.sort_values('round')
        plt.plot(global_eval['round'], global_eval['reward'], color='gold', 
                 linewidth=4, marker='*', markersize=15, 
                 label='Aggregated Global Model (Eval)',
                 path_effects=None) # gold stands out
        
        # Add a shadow/glow effect manually
        plt.plot(global_eval['round'], global_eval['reward'], color='black', 
                 linewidth=5, alpha=0.2, zorder=1)

    # 4. Final Touches
    plt.title('Worker Diversity & Global Convergence', fontsize=18, fontweight='bold', pad=25)
    plt.xlabel('Round', fontsize=13)
    plt.ylabel('Average Reward', fontsize=13)
    
    plt.legend(loc='upper left', bbox_to_anchor=(1, 1), frameon=True, shadow=True, fontsize=11)
