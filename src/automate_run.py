...
def run_experiment():
    ...
    if pipeline_type not in pipeline_map:
        print(f"Unknown pipeline type: {pipeline_type}. Choose 'fl', 'single', 'single_visual' or 'fl_visual'.")
        sys.exit(1)
    ...
    exp_id = getattr(experiment, 'experiment_id', None) or getattr(experiment, 'id', None)
    ...
    try:
        ...
        while True:
            try:
                ...
                status = getattr(r, 'state', None) or (r.run.status if hasattr(r, 'run') else None)
                ...
                err_str = str(e).lower()
                if "failed to establish" in err_str or "connection refused" in err_str or "connection reset" in err_str:
                    conn_errors += 1
