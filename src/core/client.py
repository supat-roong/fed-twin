...
class TwinClient(fl.client.NumPyClient):
    ...
    def fit(self, parameters, config):
        ...
        if self.eval_only:
            print(
                f"Twin {self.twin_id} [Round {sv_round}] [METRIC] EVAL-ONLY-SKIP Reward: 0.0 Loss: 0.0"
            )
        ...
        states, actions, rewards, episode_ends = self.env.collect_experience(
            self.model, n_episodes=episodes
        )
        ...
        log_probs = torch.log(
            action_probs.gather(1, action_t.unsqueeze(1)).squeeze() + 1e-8
        )
        ...
        return (
            ...
            len(states),
            {"reward": avg_reward, "loss": avg_loss},
        )
