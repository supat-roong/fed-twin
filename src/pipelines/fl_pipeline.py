...
spec:
  pytorchReplicaSpecs:
    ...
    timeout_seconds = max(6000, expected_duration)  # At least 100 minutes, or calculated duration
    print(f"Timeout set to {timeout_seconds} seconds (~{timeout_seconds//60} minutes) based on {fl_rounds} rounds")
    ...
    try:
        for line in process.stdout:
            line_count += 1
            if line_count % 100 == 0:
                print(f"Processed {line_count} log lines, found {metric_count} metrics...")
            
            # Check timeout
            elapsed = time.time() - start_time
            if elapsed > timeout_seconds:
                print(f"⚠️ Timeout reached after {elapsed:.0f} seconds. Processed {line_count} lines, found {metric_count} metrics.")
                break
            ...
            if match:
                ...
                if metric_count % 10 == 0 or metric_count <= 5:
                    print(f"✓ Metric #{metric_count}: R={rd}, Twin={twin_id}, Mode={csv_mode}, Rew={reward}")
        ...
        process.terminate()
        print(f"Log streaming finished. Total: {line_count} lines processed, {metric_count} metrics captured")
