...
def test_run_experiment_fl_success(mock_file, mock_sleep, mock_time, mock_exists, mock_system, mock_kfp_client):
    # Setup mocks
    ...
    mock_client_instance.get_run.side_effect = [mock_run_status_running, mock_run_status_succeeded]
...
@patch('sys.argv', ['automate_run.py', 'fl'])
def test_run_experiment_connection_failed(mock_kfp_client):
...
@patch('sys.argv', ['automate_run.py', 'fl'])
def test_run_experiment_compilation_failed(mock_system, mock_kfp_client):
...
@patch('sys.argv', ['automate_run.py', 'fl'])
def test_run_experiment_yaml_missing(mock_exists, mock_system, mock_kfp_client):
    with pytest.raises(SystemExit) as e:
        automate_run.run_experiment()
    assert e.value.code == 1
...
def test_run_experiment_invalid_type(mock_exists, mock_system, mock_kfp_client):
    with pytest.raises(SystemExit) as e:
        automate_run.run_experiment()
    assert e.value.code == 1
...
def test_run_experiment_poll_failure(mock_time, mock_sleep, mock_exists, mock_system, mock_kfp_client):
    mock_client_instance = mock_kfp_client.return_value
